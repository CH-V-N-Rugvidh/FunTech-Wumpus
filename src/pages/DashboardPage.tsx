...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen text-white pt-20 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
            <div className="flex items-center space-x-3 sm:space-x-4">
              <Monitor className="w-8 h-8 sm:w-10 sm:h-10 text-blue-400 pulse-glow" />
              <div>
                <h1 className="text-2xl sm:text-4xl font-bold gradient-text">FunTech Wumpus</h1>
                <p className="text-blue-200 text-sm sm:text-lg">Live Event Dashboard</p>
              </div>
            </div>
            <div className="text-center sm:text-right">
              ...
              <div className="text-blue-200 text-sm sm:text-lg">
      ...
      {/* Stats Bar */}
      <div className="glass border-b border-white/10 p-4 sm:p-6 mt-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="glass-dark rounded-xl p-4 sm:p-6 text-center hover:scale-105 transition-transform duration-300">
              <Users className="w-8 h-8 sm:w-10 sm:h-10 text-green-400 mx-auto mb-3 pulse-glow" />
              <div className="text-2xl sm:text-3xl font-bold">{activeCount}</div>
              <div className="text-sm text-white/70">Active Players</div>
            </div>
            <div className="glass-dark rounded-xl p-4 sm:p-6 text-center hover:scale-105 transition-transform duration-300">
              <Trophy className="w-8 h-8 sm:w-10 sm:h-10 text-yellow-400 mx-auto mb-3 pulse-glow" />
              <div className="text-2xl sm:text-3xl font-bold">{completedCount}</div>
              <div className="text-sm text-white/70">Completed</div>
            </div>
            <div className="glass-dark rounded-xl p-4 sm:p-6 text-center hover:scale-105 transition-transform duration-300">
              <Clock className="w-8 h-8 sm:w-10 sm:h-10 text-blue-400 mx-auto mb-3 pulse-glow " />
              <div className="text-2xl sm:text-3xl font-bold">{players.length}</div>
              <div className="text-sm text-white/70">Total Players</div>
            </div>
            <div className="glass-dark rounded-xl p-4 sm:p-6 text-center hover:scale-105 transition-transform duration-300">
              <Zap className="w-8 h-8 sm:w-10 sm:h-10 text-purple-400 mx-auto mb-3 pulse-glow" />
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <h2 className="text-2xl sm:text-3xl font-bold mb-8 flex items-center space-x-3">
            ...
            {activePlayers.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {activePlayers.map((player) => (
                  <div key={player.id} className="glass-dark rounded-2xl p-4 sm:p-6 border border-white/10 hover:border-white/30 transition-all duration-300 float">
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-lg sm:text-xl font-bold text-white">{player.name}</h3>
                      <div className="text-right">
                        <div className="text-xl sm:text-2xl font-bold text-blue-400">{player.steps}</div>
                    ...
                    <div className="mb-4">
                      <WumpusGrid
                        ...
                        visitedPositions={player.visitedPositions}
                        gridSize={10}
                      />
                    </div>
              ...
              <div className="glass-dark rounded-2xl p-8 sm:p-16 text-center border border-white/10">
                <Clock className="w-20 h-20 text-white/50 mx-auto mb-6 pulse-glow" />
                <p className="text-xl sm:text-2xl text-white/70">Waiting for players to join...</p>
              </div>
            )}
          </div>
